class ActionSalvageSparkPlugCB : ActionContinuousBaseCB
{
...
class ActionSalvageSparkPlug : ActionVehicleSalvagingBase
{
    void ActionSalvageSparkPlug()
    {
        m_CallbackClass = ActionSalvageSparkPlugCB;
    ...
    {
        return VehicleSalvagingCooldownManager.GetInstance().IsOnCooldown("sparkplug", target);
    }

    void MarkWreckSalvaged(ActionTarget target)
    {
        VehicleSalvagingCooldownManager.GetInstance().MarkSalvaged("sparkplug", target);
    }

    bool IsVanillaVehicleWreck(string objType)
    {
        return VehicleSalvagingWrecks.IsSparkPlugWreck(objType);
    }
    ...
    override void OnFinishProgressServer(ActionData action_data)
    {
        ...
        {
            SendMessageToClient(player, "This wreck has already had its spark plug searched recently.");
            return;
        }

        MarkWreckSalvaged(action_data.m_Target);

        // 15% chance to find a spark plug
    ...
    void ApplyRandomSparkPlugHealth(EntityAI sparkPlug)
    {
        sparkPlug.SetHealthLevel(VehicleSalvagingConfig.GetRandomPartHealthLevel());
    }
